...
def bombeiro(img):
    nimg = limiarizar(img)
    aux = identifica(nimg)
    outimg = segmentar(img, aux)
    cv2.imwrite('bombeiro_segmentada.png', outimg)


def solda(img):
    aux = identifica(img)
    outimg = segmentar(img, aux)
    cv2.imwrite('solda_segmentada.png', outimg)


# le as imagens
img_bombeiro = cv2.imread('bombeiros4.png', 0)
img_solda = cv2.imread('solda.png', 0)
