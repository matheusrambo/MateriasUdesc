...
int imprimir_submatriz (matriz_bloco_t* submatriz){
	int i,j;
 	for (j=submatriz->bloco->col_inicio; j<=submatriz->bloco->col_fim; j++){
		  	printf("\t(%d)", j);
	}
 	printf("\n");
 	for (i=submatriz->bloco->lin_inicio; i<=submatriz->bloco->lin_fim; i++){
 		printf("(%d)", i);
 		for (j=submatriz->bloco->col_inicio; j<=submatriz->bloco->col_fim; j++){
 ...
 matriz_bloco_t **particionar_matriz (int **matriz, int mat_lin, int mat_col, int orientacao, int divisor) {
    ...
    if (orientacao) {
 	...
 	for (int i = 0; i < nro_submatrizes; i++) {
 		...
 		vsubmatriz[i] = (matriz_bloco_t *) malloc (sizeof(matriz_bloco_t)); //??? wtf
		...
		//essa parte até que vai, mas não sei bem ao certo o por que
 		...
 		gerar_submatriz (matriz, *vsubmatriz, bloco_tmp);
 	}
 return vsubmatriz;
}
