...
Proof.
  ...
  - simpl. assert (HN0: n * 0 = 0).
    {induction n as [| n' IHn']. reflexivity.
    - simpl. rewrite IHn'. reflexivity. }
    rewrite HN0. reflexivity.
  - (* m = S m'*) simpl. rewrite mult_nm. rewrite IH.
    reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. rewrite mult_0_r. rewrite -> mult_0_r. simpl. rewrite -> mult_0_r. reflexivity.
  - rewrite mult_nm. rewrite IHp. rewrite mult_nm. rewrite plus_comm. rewrite plus_assoc.
    rewrite mult_nm. rewrite -> plus_assoc. assert(H: n + n*p = n*p + n). 
    { rewrite <- plus_comm. reflexivity. } rewrite -> H. rewrite <- plus_assoc.
    assert(I: m*p + (n+m) = (n+m) + m*p). {rewrite plus_comm. reflexivity. }
    rewrite <- plus_assoc. rewrite I. rewrite plus_assoc. rewrite plus_assoc. reflexivity.
  Qed. 
